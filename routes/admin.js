...
router.get('/patients/fetch', function(request, response, next) {
  ...
  async.waterfall([
    function(callback) {
      User.count({groups: group}, function(error, count) {
    ...
    function(count, callback2) {
      ...
      var query = User.find({groups: group})
        .limit(limit)
        .skip(start);
...
router.post('/patients/push', function(request, response, next) {
...
router.get('/hospital/select', function(request, response, next) {
  Hospital.find({active: true}, function(error, hosps) {
    if (error) {
...
router.post('/admins/push', function(request, response, next) {
  var operation = request.body.oper,
    ...
    if (operation == 'del') {
      ...
      async.waterfall([
        function(callback) {
          User.count({_id: {$ne: id}, hospital: hospital, groups: group},
          function(error, count) {
            callback(error, count);
          });
        },
